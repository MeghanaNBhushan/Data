...
Feature: Full analysis using compile_commands.json
  ...
  Scenario Outline: Report is created with defined configuration
    ...
    And a json config for <config_key>
      | key                                 | value                                                                                      |
      | COMPILE_COMMANDS_BUILD_COMMAND      | powershell.exe -file %(project_root)s/cmake_build.ps1 -compiler msvc -target variant1 -g   |
      | COVERITY_PROJECT_PATH               | %(test_root)s/test/integration/bin/%(config_key)s_compile_commands                         |
      | EXPORT_FORMATS                      | csv                                                                                        |
      | COMPILE_COMMANDS_JSON               | %(project_root)s/build/variant1_msvc/compile_commands.json                                 |
