...
class Compiler(compiler.CompilerBase):
    ...
    def __init__(self):
        ...
        wregex = r"\[-W(.*)\]"
        super(Compiler, self).__init__(name, regex, regex_unknown_warnings, False, wregex)
        ...
        for line in lines:
            ...
            for match in matches:
                ...
                if warning_name_match:
                    ...
                    LOGGER.debug("Extracted warning name '%s' from message '%s'", warning_name, match[3].strip())
                else:
                    LOGGER.warning("Did not find warning name match for warning '%s'", match[3])
                warnings.append(compiler_warning.CompilerWarning(match[0], int(match[1]), int(match[2]), match[3].strip(), warning_name,
                                                                 domain='compiler'))
            if not matches:
                ...
                for match in matches:
                    ...
                    if warning_name_match:
                        warning_name = warning_name_match.group(1)
                        LOGGER.debug("Extracted warning name '%s' from message '%s'", warning_name, match.strip())
                    else:
                        LOGGER.warning("Did not find warning name match for warning '%s'", match)
                    warnings.append(
                        compiler_warning.CompilerWarning("UnKnownWarnings", "NA", match.strip(), warning_name, domain='compiler'))
        return warnings
