...
Feature: Full analysis
  ...
  Scenario Outline: Report is created
    ...
    And a json config for <config_key>
      | key                    | value                                                                                    |
      | BUILD_COMMAND          | powershell.exe -file %(project_root)s/cmake_build.ps1 -compiler msvc -target variant1    |
      | COVERITY_PROJECT_PATH  | %(test_root)s/test/integration/bin/%(config_key)s_full                                   |
      | EXPORT_FORMATS         | csv                                                                                      |
