 ...
 */
#if !defined(__QACPP_MAJOR__) || (__QACPP_MAJOR__ < 44)
#ifndef __XC32__
...
constexpr inline int __builtin_clz_impl(_T __d)
{
  int __c = 0;
  _T __mask = 1ull << (sizeof (_T) * 8 - 1);
  while (__mask)
  {
    if (__d & __mask)
    {
...
constexpr inline int __builtin_clz(unsigned int __d)
{
  return __builtin_clz_impl(__d);
}
constexpr inline int __builtin_clzl(unsigned long __d)
{
  return __builtin_clz_impl(__d);
}
constexpr inline int __builtin_clzll(unsigned long long __d)
{
  return __builtin_clz_impl(__d);
}
#endif
...
/* Based on DS50001686J (2.10) and DS50002799A (2.15) */
